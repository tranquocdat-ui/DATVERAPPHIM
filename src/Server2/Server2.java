...
public class Server2 {
    ...
    public static class sv2 implements Runnable {
        ...
        public void runServer() {
            try {
                ...
                Server2.log("Chấp nhận kết nối từ " + destName + " tại cổng " + destPort + ".\n");
                ...
                boolean finished = false;
                {
                    ...
                    if (type.equals("Synchronymed") && (start == 4)) {
                        Server2.log("Hoàn tất giao dịch đặt vé. Kết thúc vòng tròn ảo.\n\n");
                    }

                    // xu ly thong tin updated va quay vong
                    if (type.equals("Updated") && (start == 4)) {
                        int stt = start;
                        Server2.log("Kết thúc quá trình cập nhật, kiểm tra đồng bộ hóa TT và Quay vòng ngược.\n\n");
                        stt = 1;
                        act += 1;
                        try {
                            int tam = pos - 2;
                    ...
                    if (type.equals("Temped") && (start == 4)) {
                        int stt = start;
                        Server2.log("Kết thúc tạo bảng tạm, cập nhật CSDL chính Quay vòng ngược.\n\n");
                        stt = 1;
                        act += 1;
                        try {
                    ...
                    if (type.equals("Locked") && (start == 4)) {
                        int stt = start;
                        Server2.log("Kết thúc khóa trường dữ liệu, tạo bảng tạm và Quay vòng ngược.\n\n");
                        stt = 1;
                        act += 1;
                        try {
                            int tam = pos - 2;
                    ...
                    if (start == 0) {
                        ...
                        Server2.log("Reply: " + replyMessage + "\n");
                        Server2.log("Thực hiện khóa trường DL. Chuyển thông điệp.\n\n");
                        try {
                            Connect co = new Connect(rount.table[vt].destination, rount.table[vt].port,
                                    rount.table[vt].name);
                            co.connect();
                    ...
                    if (type.equals("Locked") && (start != 4)) {
                        Server2.log("Chuyển thông điệp, thực hiện khóa trường DL.\n\n");
                        start++;
                        try {
                            Connect co = new Connect(rount.table[vt].destination, rount.table[vt].port,
                                    rount.table[vt].name);
                    ...
                    if (type.equals("Temped") && (start != 4)) {
                        Server2.log("Chuyển thông điệp, thực hiện tạo bảng tạm CSDL.\n\n");
                        start++;
                        try {
                            int tam = pos - 2;
                            if (tam < 0) {
                    ...
                    if (type.equals("Updated") && (start != 4)) {
                        Server2.log("Chuyển thông điệp, thực hiện cập nhật bảng chính CSDL.\n\n");
                        start++;
                        try {
                            Connect co = new Connect(rount.table[vt].destination, rount.table[vt].port,
                                    rount.table[vt].name);
                    ...
                    if (type.equals("Synchronymed") && (start != 4)) {
                        Server2.log("Chuyển thông điệp, kiểm tra quá trình đồng bộ hóa các tiến trình.\n\n");
                        start++;
                        try {
                            int tam = pos - 2;
                            if (tam < 0) {
        ...
        public void run() {
            ...
            try {
                // Đổi thành Server2 và Port 2002
                GetState gs = new GetState("Server2");
                gs.getCurrentCircle();
                gs.sendUpdate("127.0.0.1", 2002, "Server2");

                ServerSocket server = new ServerSocket(2002);
                while (true) {
                    int localPort = server.getLocalPort();
                    Server2.log("Server 2 đang lắng nghe tại cổng " + localPort + ".\n");
                    Socket client = server.accept();
                    
                    // Gắn pos = 2 cho Server 2
        ...
        public void run() {
            ...
            try {
                ...
                while (true) {
                    ...
                    Server2.log("Đang chờ kết nối lấy Lamport...\n");
                    ...
                    if (temp.startsWith("#")) {
                        try {
                            ...
                            Server2.log("Lỗi: No connect to multiServer\n");
                        }
    ...
    public static void main(String args[]) throws Exception {
        ...
        Server2.log("--- Hệ thống Server 2 đã sẵn sàng ---\n");
        Server2.log("Web Monitor đang chạy tại cổng 8080...\n");
