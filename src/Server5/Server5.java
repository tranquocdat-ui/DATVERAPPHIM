package Server5; // Khai báo đúng gói Server5
...
public class Server5 {
    ...
    public static class sv5 implements Runnable {
        ...
        public void runServer() {
            try {
                ...
                Server5.log("Chấp nhận kết nối từ " + destName + " tại cổng " + destPort + ".\n");
                ...
                boolean finished = false;
                {
                    ...
                    if (type.equals("Synchronymed") && (start == 4)) {
                        Server5.log("Hoàn tất giao dịch đặt vé. Kết thúc vòng tròn ảo.\n\n");
                    }
                    ...
                    if (type.equals("Temped") && (start == 4)) {
                        int stt = start;
                        Server5.log("Kết thúc tạo bảng tạm, cập nhật CSDL chính Quay vòng ngược.\n\n");
                        stt = 1;
                        act += 1;
                        try {
                    ...
                    if (start == 0) {
                        ...
                        Server5.log("Reply: " + replyMessage + "\n");
                        Server5.log("Thực hiện khóa trường DL. Chuyển thông điệp.\n\n");
                        try {
                            Connect co = new Connect(rount.table[vt].destination, rount.table[vt].port,
                                    rount.table[vt].name);
                            co.connect();
                    ...
                    if (type.equals("Locked") && (start != 4)) {
                        Server5.log("Chuyển thông điệp, thực hiện khóa trường DL.\n\n");
                        start++;
                        try {
                            Connect co = new Connect(rount.table[vt].destination, rount.table[vt].port,
                                    rount.table[vt].name);
                    ...
                    if (type.equals("Updated") && (start != 4)) {
                        Server5.log("Chuyển thông điệp, thực hiện cập nhật bảng chính CSDL.\n\n");
                        start++;
                        try {
                            Connect co = new Connect(rount.table[vt].destination, rount.table[vt].port,
                                    rount.table[vt].name);
                    ...
                    if (type.equals("Synchronymed") && (start != 4)) {
                        Server5.log("Chuyển thông điệp, kiểm tra quá trình đồng bộ hóa các tiến trình.\n\n");
                        start++;
                        try {
                            int tam = pos - 2;
                            if (tam < 0) {
        ...
        public void run() {
            ...
            try {
                // Đổi thành Server5 và Port 2005
                GetState gs = new GetState("Server5");
                gs.getCurrentCircle();
                gs.sendUpdate("127.0.0.1", 2005, "Server5");

                ServerSocket server = new ServerSocket(2005);
                while (true) {
                    int localPort = server.getLocalPort();
                    Server5.log("Server 5 đang lắng nghe tại cổng " + localPort + ".\n");
                    Socket client = server.accept();

                    // Gắn pos = 5 cho Server 5
                    apps.handler(client, "Server5", 5, currentCircle, hash);
                    apps.runServer();
        ...
        public void run() {
            ...
            try {
                ...
                while (true) {
                    ...
                    Server5.log("Đang chờ kết nối lấy Lamport...\n");
                    ...
                    if (temp.startsWith("#")) {
                        try {
                            ...
                            Server5.log("Lỗi: No connect to multiServer\n");
                        }
